...
public class BookingmanagementImpl extends AbstractComponentFacade implements Bookingmanagement {
	...
	private void sendInviteEmailToGuest(InvitedGuestEntity guest, BookingEntity booking) {
		...
		try {
			...
			invitedMailContent.append(booking.getEmail())
					.append(" has invited you to an event on My Thai Star restaurant").append("\n");
	...
	private void sendConfirmationEmailToHost(BookingEntity booking) {
		...
		try {
			...
			hostMailContent.append("Hi ").append(booking.getName()).append(" your booking has been confirmed.")
					.append("\n");
	...
	private void sendConfirmationActionToHost(BookingCto booking, InvitedGuestEto guest, String action) {
		...
		try {
			...
			mailContent.append(guest.getEmail()).append(" has ").append(action)
					.append(" your invitation for the event on ").append(booking.getBooking().getBookingDate())
					.append("\n");
